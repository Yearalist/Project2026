...
namespace ToySiege.Enemy
{
    ...
    public class PoliceCarEnemy : MonoBehaviour, ToySiege.Combat.IDamageable
    {
        ...
        // OverlapSphere NonAlloc buffer — GC allocation'ı önler
        private static readonly Collider[] _alertBuffer = new Collider[32];

        // Oyuncu component cache'leri — OnTriggerEnter'da her seferinde GetComponent çağırmayı önler
        private ToySiege.Player.Health.PlayerHealth _cachedPlayerHealth;
        private Rigidbody _cachedPlayerRb;
        private CharacterController _cachedPlayerCC;
        private bool _playerComponentsCached = false;
        ...
        void Start()
        {
            ...
            // Oyuncuyu bul ve component'lerini cache'le
            var playerObj = GameObject.FindGameObjectWithTag("Player");
            if (playerObj != null)
            {
                _player = playerObj.transform;
                CachePlayerComponents(playerObj);
            }
        ...
        void DoPatrol()
        {
            ...
            // Path hesaplanıyorsa bekle
            if (_agent.pathPending) return;

            // Hedef yoksa veya ulaştıysa yeni hedef ata
            if (!_agent.hasPath || _agent.remainingDistance < waypointThreshold)
            {
        ...
        public void TakeDamage(float damage, Vector3 hitPoint, Vector3 hitDirection)
        {
            ...
            if (IsDead)
            {
                ...
                // Patlama efekti — varsa partikülü oynat
                if (dashParticle != null) dashParticle.Play();
                Debug.Log("<color=red>[PoliceCar] YIKILDI!</color>");

                // Ragdoll / parçalanma desteği — ileride eklenebilir
        ...
        void OnTriggerEnter(Collider other)
        {
            ...
            // İlk çarpışmada cache'le (lazy init)
            if (!_playerComponentsCached)
            ...
            if (_cachedPlayerRb != null)
            {
                ...
                knockDir.y = 0;
                _cachedPlayerRb.AddForce(knockDir * force + Vector3.up * knockUpForce, ForceMode.Impulse);
            }

            if (_cachedPlayerCC != null && _cachedPlayerRb == null)
            {
        ...
        void AlertNearbyEnemies()
        {
            ...
            int hitCount = Physics.OverlapSphereNonAlloc(transform.position, alertRadius, _alertBuffer);
            int count = 0;

            for (int i = 0; i < hitCount; i++)
            {
                var col = _alertBuffer[i];
        ...
        void CachePlayerComponents(GameObject playerObj)
        {
            _cachedPlayerHealth = playerObj.GetComponent<ToySiege.Player.Health.PlayerHealth>();
            _cachedPlayerRb = playerObj.GetComponent<Rigidbody>();
            _cachedPlayerCC = playerObj.GetComponent<CharacterController>();
            _playerComponentsCached = true;
        }
